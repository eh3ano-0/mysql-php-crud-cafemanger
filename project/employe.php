...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['action'])) {
    if ($_POST['action'] == "add_employee") {
        ...
        $Salary = $_POST['salary'];
        $Datehir = $_POST['datehir'];
        $Accnumber = $_POST['accnumber'];

        try {
            // اجرای پروسیجر افزودن کارمند
            $sql = "CALL add_employe(?, ?, ?, ?, ?)";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("ssisi", $id, $Position, $Salary, $Datehir, $Accnumber);
            if ($stmt->execute()) {
...
<div class="container mt-5">
    ...
    <form method="POST" action="">
        ...
        <div class="row">
            <div class="col-md-6 mb-3">
                <label for="position" class="form-label">نقش</label>
        ...
        <div class="row">
            <div class="col-md-6 mb-3">
                <label for="datehir" class="form-label">تاریخ استخدام</label>
                <input type="Date" class="form-control" id="datehir" name="datehir" required>
            </div>
            <div class="col-md-6 mb-3">
                <label for="accnumber" class="form-label">شماره اکانت</label>
    ...
    <!-- جدول اطلاعات -->
    <h3 class="mt-4">داده‌های کارمند</h3>
            ...
            <tbody>
                <?php foreach ($employees as $employee): ?>
                    <tr>
                        ...
                        <td><?php echo $employee['name'] . " " . $employee['lastname']; ?></td>
                        <td><?php echo htmlspecialchars($employee['position']); ?></td>
                        <td><?php echo htmlspecialchars($employee['salary']); ?></td>
                        <td><?php echo htmlspecialchars($employee['datehir']); ?></td>
                        <td><?php echo htmlspecialchars($employee['accnumber']); ?></td>
                        <td>
                        ...
                        <td>
                            <form method="POST" action="">
                                ...
                                <input type="hidden" name="id" value="<?php echo htmlspecialchars($employee['empID']); ?>">
...
<div class="modal" id="edit-modal">
    ...
    <form method="POST" action="">
        ...
        <label >نقش</label>
        <input type="text" id="edit-position" name="position" placeholder="نقش" required>
        
        <label >حقوق</label>
        <input type="Number" id="edit-salary" name="salary" placeholder="حقوق" required>

        <label >تاریخ استخدام</label>
        <input type="Date" id="edit-datehir" name="datehir" placeholder="تاریخ استخدام" required>
        
        <label >شماره اکانت</label>
        <input type="Number" id="edit-accnumber" name="accnumber" placeholder="شماره اکانت" required>
...
<script>
    function openEditModal(employee) {
            document.getElementById('edit-id').value = employee.empID;
            document.getElementById('edit-position').value = employee.position;
            document.getElementById('edit-salary').value = employee.Salary;
            document.getElementById('edit-datehir').value = employee.datehir;
            document.getElementById('edit-accnumber').value = employee.accnumber;
